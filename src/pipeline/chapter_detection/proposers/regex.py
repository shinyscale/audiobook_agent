...
class RegexProposer(BaseProposer):
    ...
    def propose_from_toc(
        self,
        text: str,
        profile: Optional[DocumentProfile] = None,
    ) -> list[ChapterProposal]:
        """
        Generate proposals guided by TOC entries.

        When a TOC exists, we know exactly what chapter markers to look for.
        This is more reliable than pattern-matching with arbitrary spacing requirements.

        The approach:
        1. Parse TOC entries (already done in profiler)
        2. Generate patterns from TOC entry titles (e.g., "I", "II", "Chapter 1")
        3. Search for those exact patterns after the TOC region
        4. Match in document order (I before II before III...)
        """
        if not profile or not profile.table_of_contents:
            return []

        toc = profile.table_of_contents
        if not toc.entries:
            return []

        proposals = []
        toc_end = toc.toc_end_position or profile.front_matter_end or 0

        logger.info(f"TOC-guided detection: looking for {len(toc.entries)} entries after position {toc_end}")

        # Track the last found position to ensure we find chapters in order
        last_found_position = toc_end
        ...
        for entry in toc.entries:
            ...
            # Build pattern from TOC entry title
            # Escape regex special chars and match on own line with flexible whitespace
            ...
            # Search only after last found position (ensures we find chapters in order)
            search_region = text[last_found_position:]
            match = pattern.search(search_region)

            if match:
                position = last_found_position + match.start()

                # Determine chapter title format
                # If TOC entry is just a number (roman or arabic), format as "Chapter X"
                if re.match(r'^[IVXLC]+$', title) or re.match(r'^\d+$', title):
                ...
                proposals.append(ChapterProposal(
                    ...
                    confidence=0.95,  # High confidence - we know what to expect from TOC
                    reasoning=f"Found TOC entry '{title}' in text at position {position}",
                    is_hard_boundary=True,
                ))

                # Update last_found_position to search for next chapter after this one
                last_found_position = position + len(match.group(0))

                logger.debug(f"TOC-guided: found '{title}' at position {position}")
            else:
                logger.warning(f"TOC-guided: could not find '{title}' in text after position {last_found_position}")

        logger.info(f"TOC-guided detection: found {len(proposals)}/{len(toc.entries)} expected chapters")
        return proposals
