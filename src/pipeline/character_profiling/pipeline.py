...
class CharacterProfilingPipeline:
    ...
    def profile_existing_characters(
        self,
        ...
        """Generate profiles for already-identified characters.

        Use this when characters have already been identified by CharacterAgent
        and only profiling is needed (skip identification stage).

        Args:
            characters: Pre-identified characters to profile
            full_text: Complete document text
            chapter_map: Chapter boundaries from chapter detection
            summary_map: Chapter summaries from summary pipeline
            narrator_name: Detected narrator name (if any)
            narrative_style: Narrative style (e.g., "first-person", "third-person")
            source_file: Source file name for metadata
        ...
        """
        logger.info(
            f"Profiling {len(characters)} pre-identified characters (skipping identification)"
        )
        ...
        # Defensive: if narrator name is ambiguous, clear it
        if narrator_name and ambiguity_map.is_ambiguous(narrator_name):
            logger.warning(
                f"Narrator name '{narrator_name}' is ambiguous; clearing narrator assignment "
                "to avoid first-person contamination."
        ...
        if self.generate_rich_profiles:
            ...
            for i, char in enumerate(characters):
                logger.info(
                    f"Profiling character {i + 1}/{len(characters)}: {char.canonical_name}"
                )
                ...
                is_char_narrator = (
                    narrator_name
                    and char.canonical_name.lower() == narrator_name.lower()
                )
                summary_evidence = summary_extractor.extract_evidence(
                    char.canonical_name,
                    char.aliases,
...
def character_to_identified(char) -> IdentifiedCharacter:
    """Convert a pipeline Character (from character_extraction) to IdentifiedCharacter.

    This bridges the character_extraction and character_profiling models so that
    characters identified by CharacterAgent can be profiled by the profiling pipeline.

    Args:
        char: A Character object from character_extraction.models

    Returns:
        IdentifiedCharacter suitable for the profiling pipeline
    """
    return IdentifiedCharacter(
        ...
        brief_description=char.description,
