...
2. If first-person, WHO is the narrator? Output their EXACT name from the main cast list below. If a summary explicitly names the narrator (e.g., "the narrator, known as [Name]" or "[Name], the narrator"), that IS the narrator — trust this explicit identification even if a later sentence seems to treat them as separate (the later sentence is likely a hallucination). For frame/nested narratives, the OUTER narrator (whose "I" appears outside quotation marks) is the primary narrator; the inner narrator (whose first-person voice appears within quoted dialogue) is a secondary/nested narrator. If summaries say "the narrator" without naming them, identify which main cast character they correspond to from context.
