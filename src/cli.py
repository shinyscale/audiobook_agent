...
Examples:
  ...
  %(prog)s analyze manuscript.docx --wpm 160
    ...
    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Analyze a manuscript file")
    analyze_parser.add_argument(
        "file", type=str, help="Path to manuscript file (PDF, DOCX, EPUB, or TXT)"
    )
    analyze_parser.add_argument(
        "-o", "--output", type=str, help="Output JSON file path (default: <input>.analysis.json)"
    )
    analyze_parser.add_argument(
        "--wpm",
        type=int,
    ...
    analyze_parser.add_argument(
        "--keep-text", action="store_true", help="Include raw text in output (increases file size)"
    )
    analyze_parser.add_argument("--no-llm", action="store_true", help="Skip LLM refinement step")
    analyze_parser.add_argument(
        "--llm-model",
        type=str,
        default=None,
    ...
    analyze_parser.add_argument(
        "--character-model", type=str, default=None, help="Model for character extraction agent"
    )
    analyze_parser.add_argument(
        "--summary-model", type=str, default=None, help="Model for chapter summary agent"
    )
    analyze_parser.add_argument(
    ...
    analyze_parser.add_argument(
        "--tui", action="store_true", help="Open interactive TUI after analysis"
    )
    analyze_parser.add_argument(
        "--write-canonical-md",
        action="store_true",
    ...
    analyze_parser.add_argument(
        ...
        action="store_true",
        help="Enable competitive multi-LLM consensus for character merge decisions. "
        "Runs 3 LLMs with different temperatures/prompts and requires 2/3 agreement.",
    )
    analyze_parser.add_argument(
        "--competitive-model",
        action="append",
    ...
    subparsers.add_parser("specs", help="Display detected system specs and recommended settings")

    # Summary command (quick view without full analysis)
    summary_parser = subparsers.add_parser("summary", help="Show quick summary of a manuscript")
    summary_parser.add_argument("file", type=str, help="Path to manuscript file")

    args = parser.parse_args()

    # Set up debug logging if requested via flag or environment variable
    debug_enabled = args.debug or os.environ.get("AUDIOBOOK_DEBUG", "").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    verbose_enabled = args.verbose or os.environ.get("AUDIOBOOK_VERBOSE", "").lower() in (
        "1",
        "true",
        "yes",
    )
...
def run_specs():
    ...
    from .system import detect_optimal_profile, detect_system_specs, format_specs_display
...
def run_analyze(args):
    ...
    if has_per_agent_models:
        from .agents.config import AgentConfig, OrchestratorConfig
        ...
        if orchestrator_config is None:
            ...
            orchestrator_config = OrchestratorConfig(
                default_model=inferred_default or "qwen2.5:32b",
    ...
    if args.competitive_consensus or competitive_models_specified:
        ...
        # Parse --competitive-model arguments into CompetitorModelConfig objects
        competitor_configs = []
        if competitive_models_specified:
            for spec in competitive_models_specified:
                parsed = _parse_competitive_model_spec(spec)
                if parsed:
...
def print_analysis_summary(result):
    ...
    print(
        f"   ({result.metadata.total_word_count:,} words at {result.metadata.words_per_minute} WPM)"
    )
...
def _parse_competitive_model_spec(spec: str):
    """
    Parse a competitive model specification string.

    Format: model[:temp][:style][@provider]

    Examples:
        - "qwen3:30b:0.5:strict" -> qwen3:30b, temp=0.5, style=strict, provider=ollama
        - "qwen2.5:32b:0.7:contextual" -> qwen2.5:32b, temp=0.7, style=contextual
        - "llama3.1:70b:0.9:inclusive@ollama" -> llama3.1:70b, temp=0.9, style=inclusive, provider=ollama
        - "gpt-4o:0.7:contextual@openai" -> gpt-4o, temp=0.7, style=contextual, provider=openai

    Returns:
        CompetitorModelConfig or None if parsing fails
    """
    from .agents.config import CompetitorModelConfig

    # Check for @provider suffix
    provider = "ollama"
    if "@" in spec:
        spec, provider = spec.rsplit("@", 1)

    # Split by colons - model name can contain colons (e.g., "qwen3:30b")
    # Format: model[:temp][:style]
    ...
    while i >= 0:
        ...
        if part.lower() in valid_styles:
            prompt_style = part.lower()
            i -= 1
            continue

        # Check if it's a temperature (float between 0 and 2)
        try:
            temp_val = float(part)
            if 0.0 <= temp_val <= 2.0:
        ...
        # Everything else is part of the model name
        # Add remaining parts as model name
        model_parts = parts[: i + 1]
        break

    if not model_parts:
        print(f"Warning: Could not parse competitive model spec: {spec}")
        return None

    model = ":".join(model_parts)

    return CompetitorModelConfig(
        model=model,
        provider=provider,
        temperature=temperature,
        prompt_style=prompt_style,
        name=f"{model}_{prompt_style}",
    )
