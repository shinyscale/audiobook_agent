...
class CharacterAgent(Agent):
    ...
    def run(self, context: AgentContext) -> AgentResult[CharacterMap]:
        ...
        # STEP 5.6: Apply disambiguation labels to same-name characters
        # After identity resolution, if characters with identical canonical names
        # were kept separate by role_conflict constraints, append disambiguation
        # labels from the constraint edge reason to make them distinct.
        logger.info("V2 Step 5.6: Applying disambiguation labels to same-name characters")
        
        all_chars = main_cast + supporting_cast
        char_count_before = len(all_chars)
        
        # Group characters by canonical name (case-insensitive)
        name_groups = {}
        ...
        # Process groups with 2+ characters (same name, kept separate)
        for name_lower, chars in name_groups.items():
            if len(chars) < 2:
                continue
            
            canonical_name = chars[0].canonical_name
            char_ids = [c.id for c in chars]
            logger.info(
                f"Found {len(chars)} characters with name '{canonical_name}': {char_ids}"
            )
            
            # Find role_conflict constraint edges between these characters
            constraint_edges = []
            for i, char_a in enumerate(chars):
                for char_b in chars[i + 1:]:
                    # Check both directions
                    edges = [
                        e for e in ig.constraint_edges
                        if e.constraint_type.value == "role_conflict"
                        and (
                            (e.source == char_a.id and e.target == char_b.id)
                            or (e.source == char_b.id and e.target == char_a.id)
                        )
                    ]
            ...
            if not constraint_edges:
                logger.info(
                    f"No role_conflict edges found for '{canonical_name}' characters - "
                    f"leaving names unchanged"
                )
                continue
            ...
            if not match:
                logger.warning(
                    f"Could not parse labels from constraint reason: {edge.reason}"
                )
                continue
            ...
            logger.info(f"Parsed {len(labels)} labels: {labels}")
            
            # Ensure we have enough labels (pad if needed)
            while len(labels) < len(chars):
                labels.append(f"variant {len(labels) + 1}")
            
            # Sort characters by mention count (descending) for consistent label assignment
            # Higher mention count gets first label
            chars_sorted = sorted(chars, key=lambda c: c.mention_count, reverse=True)
            
            # Apply labels
            for char, label in zip(chars_sorted, labels[:len(chars)]):
                old_name = char.canonical_name
                char.canonical_name = f"{old_name} ({label})"
                logger.info(
                    f"Applied label: '{old_name}' → '{char.canonical_name}' "
                    f"(char_id={char.id}, mentions={char.mention_count})"
                )
        
        # Verify no characters were lost
        all_chars_after = main_cast + supporting_cast
        char_count_after = len(all_chars_after)
        if char_count_after != char_count_before:
            logger.error(
                f"Character count changed during disambiguation! "
                f"Before: {char_count_before}, After: {char_count_after}"
            )
            logger.error(f"IDs before: {[c.id for c in all_chars]}")
            logger.error(f"IDs after: {[c.id for c in all_chars_after]}")
        else:
            logger.info(
                f"V2 Step 5.6 complete: {char_count_after} characters "
                f"(character count unchanged ✓)"
            )
